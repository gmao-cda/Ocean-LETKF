...
!===============================================================================
  ...
  !-----------------------------------------------------------------------------
  print *, "obsop_tprof.f90:: calling read_en4_nc..."
  CALL read_en4_nc(obsinfile,id_t_obs,obs_data,nobs)
  print *, "obsop_tprof.f90:: finished read_en4_nc."
  ...
  if (DO_INSITU_to_POTTEMP) then ! Need to read in climatological salinity to compute potential temperature
    ...
    CALL read_en4_nc(obsinfile2,id_s_obs,obs_data,nobs_s)
